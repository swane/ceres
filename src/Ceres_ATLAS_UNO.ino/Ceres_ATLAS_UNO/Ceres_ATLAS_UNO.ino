...
//Wheel speed
//CIRC=0.81m, 4 pulses / rev so each pulse is 0.27m, wheel_ppd=0.27
#define wheel_ppd 0.27
...
double RC_steer() {
  ...
  t = millis();
  while ((!digitalRead(STEER_RC_PIN)) && (millis() - t < 50))
  {
...
void loop2()
{
  motor_driver_steer = -RC_steer() * 30;
...
{
  pc_com = millis();
  //bearing = read_compass(); **REMOVED AS CAUSES DELAY!!
  send_data_PC();
}
if ((millis() - motor_intrpt) > MOTOR_TIME)
{
